...
def cmd_query_text(args: argparse.Namespace) -> None:
    ...
    _log_json("query-text hits:", out, level=logging.INFO)
...
def cmd_query_image(args: argparse.Namespace) -> None:
    ...
    hits = query(col, qemb, top_k=args.top_k)
    out = [h.__dict__ for h in hits]
    _log_json("query-image hits:", out, level=logging.INFO)
...
def cmd_eval(args: argparse.Namespace) -> None:
    cfg = EvalConfig(
        ...
        qwen_local_root=args.qwen_local_root,
        top_k=args.top_k,
        evidence_k=args.evidence_k,
        use_nir_for_query=args.use_nir,
        include_nir_evidence=args.include_nir_evidence,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # common logging args for each subcommand
...
def main() -> None:
    ...
    # setup logging ASAP after parsing args
    setup_logging(
    ...
    try:
        ...
        # allow argparse/sys.exit
        raise
    except Exception:
        run_logger.exception("Command failed: %s", args.cmd)
        # exit without printing traceback to terminal (we already logged it)
        sys.exit(1)
